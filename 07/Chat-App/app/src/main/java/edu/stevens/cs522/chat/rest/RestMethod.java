...
public class RestMethod {
    ...
    private URL registerURL(RegisterRequest request) {
        ...
        return fromURI(String.format("%s/chat/%s?%s=%s", baseURI, clientID.toString(), CHAT_NAME, request.chatName));
    }

    private URL postMessageURL() {
        String baseURI = Settings.getServerUri(context);
        UUID clientID = Settings.getClientId(context);
        return fromURI(String.format("%s/chat/%s/messages", baseURI, clientID.toString()));
    }

    private URL syncMessagesURL(SynchronizeRequest request) {
        String baseURI = Settings.getServerUri(context);
        UUID clientID = Settings.getClientId(context);
        return fromURI(String.format("%s/chat/%s/sync?%s=%s", baseURI, clientID.toString(), LAST_SEQUENCE_NUMBER, Long.toString(request.lastSequenceNumber)));
    }
